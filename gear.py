...
#@-<<declarations>>
#@+others
#@+node:.20150428201639.1889: ** class Gear
class Gear(object):
    #@+others
    #@+node:.20150428201639.1890: *3* __init__
    def __init__(self):
        ...
        if not os.path.isdir(download_root_dir+"tmp"):
            try:
                ...
                print("mkdir error")
    #@+node:.20150428201639.1891: *3* default
    @cherrypy.expose
    def default(self, attr='default', *args, **kwargs):
        raise cherrypy.HTTPRedirect("/")
    #@+node:.20150428201639.1892: *3* index
...
'''
        ...
        return outstring
    #@+node:.20150428201639.1893: *3* interpolation
    @cherrypy.expose
    ...
    #@+node:.20150428201639.1894: *3* gear_width
    # 改寫為齒面寬的設計函式
    @cherrypy.expose
    def gear_width(self, horsepower=100, rpm=1000, ratio=4, toothtype=1, safetyfactor=2, material_serialno=1, npinion=18):
        ...
        return outstring
    #@+node:.20150428201639.1895: *3* cube_weblink
    @cherrypy.expose
    ...
    '''
        return outstring
    #@+node:.20150428201639.1896: *3* gear_weblink
    @cherrypy.expose
